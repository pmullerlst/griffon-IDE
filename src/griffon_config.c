...
void ui_done (void)
{
  ...
  //glist_save_to_file (gl_famous, confile.tea_gl_famous);
  //glist_save_to_file (gl_kwas_history, confile.tea_kwas_history);
  //glist_save_to_file (gl_tea_kwas_bookmarks, confile.tea_kwas_bookmarks);
  //glist_save_to_file (gl_autocomp, confile.tea_autocomp);
...
void reload_hotkeys (void)
{
  ...
  ht_tea_hotkeys = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, g_free);

  GList *list = parse_conf_file_to_list (confile.tea_hotkeys);
  ...
  while (p)
        {
         ...
         if (a[1])
            shortcut = g_utf8_strup (a[1], -1);

         if (a[0])
            menuitem = g_strdup (a[0]);
 
         if (shortcut && menuitem)
            g_hash_table_insert (ht_tea_hotkeys, menuitem, shortcut);  
...
/*#ifdef HAVE_LIBASPELL
  ...
  GList *l = get_speller_modules_list (g_getenv ("LANG"));  
...
void fill_entities (void)
{
  ...
  g_hash_table_insert (ht_entities, _("espace insécable"), "160");
  g_hash_table_insert (ht_entities, _("cent signe"), "162");
  g_hash_table_insert (ht_entities, _("pound signe"), "163");
  g_hash_table_insert (ht_entities, _("signe de la monnaie"), "164");
  g_hash_table_insert (ht_entities, _("yen signe"), "165");
  g_hash_table_insert (ht_entities, _("section signe"), "167");
  g_hash_table_insert (ht_entities, _("copyright signe"), "169");
  g_hash_table_insert (ht_entities, _("une marque signe"), "8482");
  g_hash_table_insert (ht_entities, _("féminin indicateur ordinal \"ª\""), "170");
  g_hash_table_insert (ht_entities, _("pointant vers la gauche à double guillemet angle"), "171");
//  g_hash_table_insert (ht_entities, _("not sign"), "172");
  g_hash_table_insert (ht_entities, _("trait d'union"), "173");
  g_hash_table_insert (ht_entities, _("signe enregistré"), "174");
  g_hash_table_insert (ht_entities, _("signe de degré"), "176");
  g_hash_table_insert (ht_entities, _("plus-minus sign"), "177");
  g_hash_table_insert (ht_entities, _("pointant vers la droite à double guillemet angle"), "187");
//  g_hash_table_insert (ht_entities, _("fraction one quarter"), "188");
//  g_hash_table_insert (ht_entities, _("une fraction moitié"), "189");
//  g_hash_table_insert (ht_entities, _("fraction three quarters"), "190");
  g_hash_table_insert (ht_entities, _("gauche guillemet simple"), "8216");
  g_hash_table_insert (ht_entities, _("droit guillemet simple"), "8217");
  g_hash_table_insert (ht_entities, _("symbole de l'euro"), "8364");
  g_hash_table_insert (ht_entities, _("grec symbole pi"), "982");
  g_hash_table_insert (ht_entities, _("flèche vers la gauche"), "8592");
  g_hash_table_insert (ht_entities, _("flèche vers le haut"), "8593");
  g_hash_table_insert (ht_entities, _("flèche vers la droite"), "8594");
  g_hash_table_insert (ht_entities, _("flèche vers le bas"), "8595");
  g_hash_table_insert (ht_entities, _("racine carrée"), "8730");
  ...
  g_hash_table_insert (ht_entities, _("indicateur ordinal masculin \"º\""), "186");
  g_hash_table_insert (ht_entities, _("guillemet"), "34");
  g_hash_table_insert (ht_entities, _("&"), "38");
  g_hash_table_insert (ht_entities, _("signe inférieur à"), "60");
  g_hash_table_insert (ht_entities, _("signe supérieur à"), "62");
...
//  confile.autocomp_file = g_strconcat (confile.tea_main_dir, G_DIR_SEPARATOR_S, "autocomp_file", NULL);
  ...
  //some default values were taken from Bluefish
...
//confile.do_enc_autodetect = ht_get_int_value ("do_enc_autodetect", 0);
